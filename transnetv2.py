...
def main():
    ...
    parser.add_argument("folder", type=str, help="path to video folders to process")
    parser.add_argument("--start", default=0, type=int, help="start")
    parser.add_argument("--end", default=-1, type=int, help="end")
    args = parser.parse_args()
    model = TransNetV2(None)
    if args.end == -1:
      input_dir = sorted(glob.glob(args.folder))[args.start:]
    if args.end >= 0:
      input_dir = sorted(glob.glob(args.folder))[args.start : args.end + 1]
    for i in range(len(input_dir)):
        input_dir[i] = str.replace(input_dir[i], '\\', '/')
    output_dir = '/content/drive/MyDrive/AIC-2025/Database/scenes_txt'
    for file in input_dir:
        video_id = file.split('/')[-1][:-4]
        if os.path.exists(output_dir + '/' + video_id + '.txt'):
            print(f"[TransNetV2] {video_id} already exists. "
                    f"Skipping get frame of video {file}.", file=sys.stderr)
            continue
        video_frames, single_frame_predictions, all_frame_predictions = model.predict_video(file)
        scenes = model.predictions_to_scenes(single_frame_predictions)
        save_dir = output_dir + '/' + video_id + '.txt'
        np.savetxt(save_dir, scenes, encoding= 'utf-8', fmt='%d')
